...
When running `shAIm.py` for the first time, the hidden folder `.shAIm` will be created in your home directory. This folder contains `config.json` where you can set profile settings, swarm settings, agent definitions, default skill names, and attachment size limits. We recommend updating `config.json` immediately after running `shAIm.py` for the first time. The hidden folder `.shAIm` also contains conversations, uploads, schedules and skills.
